...
import { useMessagesStore, useAuthStore } from '@/lib/stores';
...
function shouldShowHeader(messages: MessagePayload[], index: number): boolean {
  if (index === messages.length - 1) return true; // oldest message (list is inverted)
  const current = messages[index];
  const prev = messages[index + 1]; // previous in display order (older)
  ...
  return diff > 5 * 60 * 1000; // 5 minutes apart
}
...
export function ChannelChatScreen({ route }: Props) {
  ...
  const messages = useMessagesStore((s) => s.channels[channelId]?.messages ?? []);
  const hasMore = useMessagesStore((s) => s.channels[channelId]?.hasMore ?? true);
  const loading = useMessagesStore((s) => s.channels[channelId]?.loading ?? false);
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        ...
        setMessages(channelId, data, data.length === 50);
      } catch (err) {
  ...
  const loadMore = useCallback(async () => {
    ...
    try {
      ...
      if (data.length < 50) {
        useMessagesStore.getState().setHasMore(channelId, false);
      }
    } catch (err) {
  ...
  const handleSend = useCallback(async () => {
    ...
    setText('');
    try {
      await apiClient.post(`/channels/${channelId}/messages`, { content });
      // Message will arrive via gateway and be added to store
    } catch (err) {
      console.error('[Chat] Failed to send message:', err);
      setText(content); // Restore on failure
    }
    setSending(false);
  }, [channelId, text, sending]);
    ...
    >
      ...
      <View style={styles.composer}>
        <TextInput
          ...
          placeholder={`#${route.params.channelName} kanalına mesaj gönder`}
...
const styles = StyleSheet.create({
  ...
  messageCompact: {
    paddingHorizontal: spacing[4],
    paddingLeft: spacing[4] + 32 + spacing[2.5], // avatar width + gap
  ...
  emptyChat: {
    ...
    // Inverted list flips this — appears at bottom
    transform: [{ scaleY: -1 }],
  },
  composer: {
    flexDirection: 'row',
