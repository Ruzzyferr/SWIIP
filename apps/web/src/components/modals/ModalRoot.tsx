...
export function ModalRoot() {
  ...
  return (
    <Modal
      ...
      onClose={closeModal}
      title={getTitle()}
      showClose={activeModal?.type !== 'create-guild' && activeModal?.type !== 'join-guild' && activeModal?.type !== 'create-channel' && activeModal?.type !== 'user-profile' && activeModal?.type !== 'create-dm' && activeModal?.type !== 'forward-message' && activeModal?.type !== 'create-thread'}
      size={activeModal?.type === 'invite' || activeModal?.type === 'user-profile' || activeModal?.type === 'create-dm' || activeModal?.type === 'forward-message' || activeModal?.type === 'create-thread' ? 'md' : 'sm'}
    >
      {renderContent()}
    </Modal>
  );
